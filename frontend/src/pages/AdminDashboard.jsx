...
export default function AdminDashboard() {
  ...
  const changeVolatility = async (stock) => {
    ...
    if (!v || isNaN(v)) return toast.error("Invalid number");

    try {
      await adminUpdateVolatility({
        stockId: stock._id,
        volatility: Number(v)
      });

      toast.success("Volatility Updated!");
      loadStocks();
    } catch (err) {
      console.error(err);
    ...
    dark:text-white">
      ...
      <div className="bg-gray-800/40 border border-gray-700 rounded-2xl p-6 w-full max-w-xl mb-10">
        ...
        <input
          ...
          placeholder="Volatility (e.g. 1.2)"
          value={volatility}
      ...
      <table className="w-full bg-gray-800/40 border border-gray-700 rounded-xl overflow-hidden">
        ...
        <tbody className="divide-y divide-gray-700">
          {Array.isArray(stocks) &&
            stocks.map((s) => (
              <tr key={s._id} className="hover:bg-gray-700/50 transition-all">
